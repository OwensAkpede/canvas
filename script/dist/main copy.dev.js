...
var error_msg = 'something unusual here',
    transform_speed = 2,
    canvas_types = {
  IMAGEDATA: "imagedata",
  TEXT: "text"
    ...
    canvas_state = [],
    canvas_reshape = {
  center: function center(event, elm) {
    ...
    canvas_board[canvas_events.mousemove] = function (ev) {
      ...
      e.style.left = e.offsetLeft + (ev.movementX || ev.webkitMovementX || 0) + 'px'; // e.scrollIntoViewIfNeeded(true)
    };
  },
  "right_imagedata": function right_imagedata(event, elm) {
    var PRVX;
    var PRVY;
    ...
    canvas_board[canvas_events.mousemove] = function (ev) {
      ...
      p.style.width = p.offsetWidth + (ev.movementX || ev.webkitMovementX || 0) + "px";
    };
  },
  "bottom_imagedata": function bottom_imagedata(event, elm) {
    var PRVX;
    var PRVY;
    var p = elm.parentElement;
  ...
  "bottom-right_imagedata": function bottomRight_imagedata(event, elm) {
    var p = elm.parentElement;
    var PRVX;
    var PRVY;
  ...
  "bottom-right_text": function bottomRight_text(event, elm) {
    ...
    canvas_board[canvas_events.mousemove] = function (ev) {
      ...
      p.style.fontSize = Number(p.style.fontSize.replace(/[a-z]/img, '')) + (ev.movementY || ev.webkitMovementY || 0) + "px"; // elm.style.width = p.offsetWidth+"px"//(p.offsetWidth + ((ev.movementX || ev.webkitMovementX || -1))) + "px";
    };
  },
  "right_text": function right_text(event, elm) {
    var PRVX;
    var PRVY;
    var p = elm.parentElement;
    ...
    canvas_reshape_active = function canvas_reshape_active(elm, event) {
  var d = elm.getAttribute('d');

  var _d = d + '_' + elm.parentElement.parentElement._type.toLowerCase(); // console.log(_d);
...
var canvas_ctrl = document.createElement('div');
...
function imageToCanvas(img) {
  ...
  void _zIdexCore().then(function (id) {
    ...
    _canvas_div.style.width = "".concat(_canvas_div.offsetWidth, "px");
    _canvas_div.style.height = "".concat(_canvas_div.offsetHeight, "px");
    _canvas_div.style.left = "".concat(_canvas_div.offsetLeft - _canvas_div.offsetWidth / 2, "px");
    _canvas_div.style.top = "".concat(_canvas_div.offsetTop - _canvas_div.offsetHeight / 2, "px");
...
function getText(txt, data) {
  ...
  _zIdexCore().then(function (id) {
    ...
    _canvas_div.style.width = "".concat(span.offsetWidth + 1, "px");
    _canvas_div.style.left = "".concat(_canvas_div.offsetLeft - _canvas_div.offsetWidth / 2, "px");
    _canvas_div.style.top = "".concat(_canvas_div.offsetTop - _canvas_div.offsetHeight / 2, "px");
    void _canvas_div.removeAttribute('default');
...
function loadImageData(data, id, foo, _canvas_div) {
  ...
  _canvas_div.style.width = "".concat(data.width, "px");
  _canvas_div.style.height = "".concat(data.height, "px");
  _canvas_div.style.left = "".concat(data.x, "px");
  _canvas_div.style.top = "".concat(data.y, "px");
  _canvas_div._id = id;
...
function generateItem(e) {// return {
  //     original_height: _canvas.height,
  //     original_width: _canvas.width,
